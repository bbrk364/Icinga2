...
BOOST_AUTO_TEST_CASE(simple)
{
	...
	BOOST_CHECK(expr->Evaluate(frame).Result == Empty);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "\n3");
	BOOST_CHECK(expr->Evaluate(frame).Result == 3);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "{ 3\n\n5 }");
	BOOST_CHECK_THROW(expr->Evaluate(frame).Result, ScriptError);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "1 + 3");
	BOOST_CHECK(expr->Evaluate(frame).Result == 4);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "3 - 1");
	BOOST_CHECK(expr->Evaluate(frame).Result == 2);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "5m * 10");
	BOOST_CHECK(expr->Evaluate(frame).Result == 3000);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "5m / 5");
	BOOST_CHECK(expr->Evaluate(frame).Result == 60);
	delete expr;
	...
	BOOST_CHECK(!expr->Evaluate(frame).Result);
	delete expr;
	...
	BOOST_CHECK(expr->Evaluate(frame).Result == 14);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "(2 + 3) * 4");
	BOOST_CHECK(expr->Evaluate(frame).Result == 20);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "2 * - 3");
	BOOST_CHECK(expr->Evaluate(frame).Result == -6);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "-(2 + 3)");
	BOOST_CHECK(expr->Evaluate(frame).Result == -5);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "- 2 * 2 - 2 * 3 - 4 * - 5");
	BOOST_CHECK(expr->Evaluate(frame).Result == 10);
	delete expr;
	...
	BOOST_CHECK(expr->Evaluate(frame).Result == (double)~(long)0);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "4 << 8");
	BOOST_CHECK(expr->Evaluate(frame).Result == 1024);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "1024 >> 4");
	BOOST_CHECK(expr->Evaluate(frame).Result == 64);
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "2 << 3 << 4");
	BOOST_CHECK(expr->Evaluate(frame).Result == 256);
	delete expr;
	...
	dict = expr->Evaluate(frame).Result;
	delete expr;
	...
	BOOST_CHECK(expr->Evaluate(frame).Result == "test3");
	delete expr;

	expr = ConfigCompiler::CompileText("<test>", "\"\\\"te\\\\st\"");
	BOOST_CHECK(expr->Evaluate(frame).Result == "\"te\\st");
	delete expr;
	...
	expr = ConfigCompiler::CompileText("<test>", "({ a = 3\nb = 3 })");
	BOOST_CHECK(expr->Evaluate(frame).Result.IsObjectType<Dictionary>());
	delete expr;
}
...
BOOST_AUTO_TEST_CASE(advanced)
{
	...
	BOOST_CHECK(expr->Evaluate(frame2).Result == Value(self));
	delete expr;
	...
	BOOST_CHECK(expr->Evaluate(frame).Result.IsEmpty());
	delete expr;
	...
	expr = ConfigCompiler::CompileText("<test>", "{{ 3 }}");
	func = expr->Evaluate(frame).Result;

...
String Logger::SeverityToString(LogSeverity severity)
{
	switch (severity) {
		...
		case LogCritical:
			return "critical";
		default:
			ThrowException(std::invalid_argument("Invalid severity."));
	}
}
...
LogSeverity Logger::StringToSeverity(const String& severity)
{
	if (severity == "debug")
		...
		ThrowException(std::invalid_argument("Invalid severity: " + severity));
}
...
void Logger::ValidateSeverity(const String& value, const ValidationUtils& utils)
{
	...
	try {
		...
		ThrowException(ValidationError(this, boost::assign::list_of("severity"), "Invalid severity specified: " + value));
	}
}
